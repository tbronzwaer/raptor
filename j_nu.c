...
real color_sphere_outer(real X_u[NDIM],int icur,real intensity[maxsize][num_indices],real tau){
        //printf("background %e %e\n",intensity[icur][0], color_outer(X_u)*exp(-tau));
        intensity[icur][0] += color_outer(X_u)*exp(-tau);
...
real color_outer(real X_u[NDIM]){
        ...
        real phi = fmod(X_u[3],2*M_PI);
        real theta = fmod(X_u[2],1.);
        ...
        int xPixel = (int) ((3000. * phi / (2. * M_PI))); //-0.5 +1000.) -1000 ;
        int yPixel = (int) ((1500. * theta / (1.))); //-0.5 + 1000.) - 1000;

        real I_bg = get_BG_color(xPixel, yPixel) / 255. * .2e-09;
...
#if (BACKGROUND_SEG)
real color_sphere_seg(real X_u[NDIM],int icur,real intensity[maxsize][num_indices]){

        if( intersect(X_u))
                intensity[icur][0] = 0;

        else
                intensity[icur][0] = color_outer(X_u);
...
int intersect(real X_u[NDIM]){
        ...
        if(phi<0)
                phi+=2.*M_PI;
        real theta = M_PI*fmod(X_u[2],1.);
        real w_theta = 0.001 * M_PI;
        real w_phi = 0.001 * M_PI;
        int n_theta_sphere = 16;
        int n_phi_sphere = 38;

        for(int i=0; i<n_theta_sphere; i++) {
                real thetaC = M_PI * ((real)i) / ((real) n_theta_sphere);
                if(theta >= (thetaC - w_theta) && theta <= (thetaC + w_theta))
                        return 1;
        }

        for(int i=0; i<n_phi_sphere; i++) {
                real phiC = 2.*M_PI * ((real)i) / ((real) n_phi_sphere);
                if(phi >= (phiC - w_phi) && phi <= (phiC + w_phi))
                        return 1;
        }
        return 0;
...
real color_outer_seg(real X_u[NDIM]){
